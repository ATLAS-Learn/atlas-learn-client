    ...
    <div className='space-y-8'>
      ...
      <div className='grid grid-cols-2 md:grid-cols-4 gap-6'>
        {[
          ...
          <div key={i} className={`${s.color} rounded-3xl p-7 hover:shadow-lg transition-all duration-200`}>
            <p className='text-sm font-medium text-gray-500'>{s.label}</p>
            <p className='text-3xl font-bold text-[#084A59] tracking-tight mt-1'>{s.value}</p>
          </div>
        ))}
      </div>
      ...
      {/* Quick Actions */}
      <div>
        <h2 className='text-lg font-bold text-[#084A59] mb-4'>Quick Actions</h2>
        <div className='grid grid-cols-1 sm:grid-cols-3 gap-6'>
          {[
            ...
            <Link key={i} to={item.to} style={{ textDecoration: 'none' }} className='bg-white rounded-3xl border border-gray-200 p-6 hover:shadow-lg hover:border-gray-300 transition-all duration-200 group'>
              <div className='text-gray-300 group-hover:text-[#084A59] transition-colors mb-4'>
                {item.icon}
              </div>
              <p className='text-base font-bold text-[#084A59] mb-1'>{item.label}</p>
