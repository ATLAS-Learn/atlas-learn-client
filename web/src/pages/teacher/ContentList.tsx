  ...
  return (
    <div className='space-y-6'>
      <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-4'>
        <div>
          <h2 className='text-2xl font-bold text-[#1F2524]'>Content Browser</h2>
          ...
          {subjects.map((subject, idx) => (
            ...
            >
              <div className='p-5'>
                <div className='flex items-start justify-between mb-3'>
                  ...
                  <div className='flex items-center gap-1'>
                    <button
                      ...
                      className='p-1.5 rounded-lg text-gray-400 hover:text-[#B8860B] hover:bg-slate-50 transition-colors'
                    >
                ...
                <h3 className='font-bold text-[#1F2524] text-lg mb-1'>{subject.name}</h3>
        ...
        <div className='fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4' onClick={() => setShowSubjectForm(false)}>
          <div className='bg-white rounded-2xl w-full max-w-lg shadow-2xl' onClick={e => e.stopPropagation()}>
            <div className='px-6 py-5 border-b border-gray-100 flex items-center justify-between'>
              <h3 className='text-lg font-bold text-[#1F2524]'>{editSubject ? 'Edit Subject' : 'New Subject'}</h3>
            ...
            <form onSubmit={handleSubjectSubmit} className='px-6 py-5 space-y-4'>
              ...
              <div className='flex gap-3 pt-2'>
                ...
                <button type='submit' className='flex-1 py-2.5 bg-[#F2B138] text-white font-bold rounded-xl hover:bg-[#996515] transition-colors text-sm'>{editSubject ? 'Update' : 'Create'}</button>
              </div>
            </form>
          </div>
        </div>
