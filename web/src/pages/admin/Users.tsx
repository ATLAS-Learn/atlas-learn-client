...
export default function AdminUsers() {
  ...
  const handleCancelInvite = async (userId: string, email: string) => {
    if (
      !confirm(
        `Cancel invite for ${email}? This will delete the pending invitation.`,
      )
    )
      return;
    try {
  ...
  const roleBadge = (role: string) => {
    ...
    return (
      <span
        className={`inline-flex px-2.5 py-0.5 rounded-lg text-xs font-semibold capitalize ${styles[role] || 'bg-gray-50 text-gray-600'}`}
      >
        {role}
      </span>
    );
        ...
        <div className='flex items-center gap-2'>
          ...
          >
            ...
            {pendingInvites.length > 0 && (
              <span className='ml-1.5 px-1.5 py-0.5 bg-[#084A59] text-white text-xs rounded-md font-bold'>
              ...
              {pendingInvites.map((inv) => (
                ...
                >
                  <div className='flex items-center gap-3'>
                    ...
                    <div>
                      <p className='text-sm font-semibold text-[#084A59]'>
                        {inv.user.name || 'Unnamed'}
                      </p>
                      <p className='text-xs text-gray-400'>{inv.user.email}</p>
                    </div>
                    <span className='px-2 py-0.5 bg-gray-100 rounded text-xs font-semibold text-gray-500 uppercase'>
                      {inv.user.role}
                    </span>
                  </div>
                  <div className='flex items-center gap-3'>
                    ...
                    <button
                      onClick={() => handleResendInvite(inv.user.id)}
                      className='text-xs font-semibold text-[#084A59] hover:text-[#011C26] transition-colors'
                    >
                      Resend
                    </button>
                    <button
                      onClick={() =>
                        handleCancelInvite(inv.user.id, inv.user.email)
                      }
                ...
                users.map((u) => (
                  ...
                  >
                    <td className='px-6 py-3.5'>
                      <div className='flex items-center gap-3'>
                        <div className='w-9 h-9 rounded-full bg-[#084A59] flex items-center justify-center text-xs font-bold text-white shrink-0'>
                    ...
                    <td className='px-6 py-3.5'>
                      <span
                        className={`inline-flex items-center gap-1.5 text-xs font-semibold ${u.isActive ? 'text-slate-600' : 'text-gray-400'}`}
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${u.isActive ? 'bg-slate-400' : 'bg-gray-300'}`}
                        />
                    ...
                    <td className='px-6 py-3.5 text-right'>
                      {u.isActive ? (
                        <button
                          ...
                          className='text-xs font-semibold text-slate-400 hover:text-slate-400 px-3 py-1.5 rounded-lg hover:bg-slate-50 transition-colors'
                        >
                          Deactivate
                        </button>
                      ) : (
                        <button
                          ...
                          className='text-xs font-semibold text-slate-500 hover:text-slate-700 px-3 py-1.5 rounded-lg hover:bg-slate-50 transition-colors'
                        >
                          Reactivate
                        </button>
                      )}
          ...
          >
            <div className='px-6 py-5 border-b border-gray-100 flex items-center justify-between'>
              <div>
                ...
                <p className='text-sm text-gray-400 mt-0.5'>
                  Send an email invitation to join Apex Learn
                </p>
              </div>
              <button
                onClick={() => setShowInviteModal(false)}
            ...
            <div className='px-6 py-5'>
              {inviteError && (
                <div className='bg-slate-50 border border-slate-200 text-slate-600 px-4 py-3 rounded-xl mb-4 text-sm font-medium'>
                  {inviteError}
                </div>
              )}
              {inviteSuccess && (
              ...
              <form onSubmit={handleInvite} className='space-y-4'>
                ...
                <div>
                  ...
                  <div className='text-sm text-gray-500 bg-gray-50 rounded-xl px-4 py-3'>
                    <span className='font-medium text-[#084A59]'>Teacher</span> — Invited users will be assigned as teachers
                  </div>
                </div>
