...
export default function AdminFeedback() {
  ...
  const handleSendReply = () => {
    ...
    updateMutation.mutate(
      {
        id: replyModal.feedback.id,
        payload: { adminReply: replyText, status: 'reviewed' },
      },
      {
        onSuccess: () => {
            ...
            <tbody>
              {feedbacks.map((fb: any) => (
                ...
                >
                  ...
                  <td className='px-5 py-4'>
                    <span
                      className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold ring-1 ${CATEGORY_COLORS[fb.category] || 'bg-gray-100 text-gray-500'}`}
                    >
                    ...
                    {fb.rating ? (
                      <div className='flex items-center gap-0.5'>
                        {[1, 2, 3, 4, 5].map((s) => (
                          <svg
                            key={s}
                            className={`w-4 h-4 ${s <= fb.rating ? 'text-[#F2B138]' : 'text-gray-200'}`}
                            fill='currentColor'
                            viewBox='0 0 20 20'
                          >
                  ...
                  <td className='px-5 py-4'>
                    <span
                      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ring-1 ${STATUS_COLORS[fb.status] || 'bg-gray-100 text-gray-500'}`}
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${STATUS_DOT[fb.status] || 'bg-gray-400'}`}
                      />
                  ...
                  <td className='px-5 py-4'>
                    <div className='flex items-center gap-2'>
                      {fb.status === 'pending' && (
                        <>
                          <button
                            onClick={() => handleChangeStatus(fb, 'resolved')}
                      ...
                      {fb.status === 'reviewed' && (
                        <>
                          ...
                          <button
                            onClick={() => handleChangeStatus(fb, 'pending')}
