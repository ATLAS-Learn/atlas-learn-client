...
export default function SuperadminLayout() {
  ...
  const currentPage = nav.find(
    (n) =>
      ...
      (n.to !== '/superadmin' && location.pathname.startsWith(n.to)),
  );
  ...
  return (
    <div className='h-screen bg-[#F0F0F0] flex overflow-hidden'>
      {/* Sidebar */}
      <aside
        className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} fixed inset-y-0 left-0 z-50 w-72 bg-[#011C26] flex flex-col transition-transform lg:translate-x-0 lg:static lg:shrink-0 shadow-xl`}
      >
        {/* Logo */}
        <div className='px-6 py-8 flex flex-col items-center border-b border-white/10'>
          ...
          <p className='text-gray-300 text-sm font-medium mt-3'>Superadmin Console</p>
        </div>
      ...
      <div className='flex-1 flex flex-col min-w-0 overflow-y-auto'>
        ...
        <header className='sticky top-0 z-30 bg-white border-b border-gray-200 shadow-sm px-6 lg:px-8 py-4 flex items-center gap-4'>
          ...
          <div className='flex-1'>
            <h1 className='text-lg font-bold text-[#011C26]'>
              {currentPage?.label || 'Superadmin'}
            </h1>
          </div>
        </header>
